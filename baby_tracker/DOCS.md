...
| `time_format`    | list            | `12h`              | Clock style for every time the app prints: `12h` (`8:30 PM`) or `24h` (`20:30`). Applies **everywhere at once** — the journal, Home Assistant notifications, pump/feed reminders, the MQTT sensors and the AI summary — because those times are built server-side. There is deliberately no per-browser toggle: a browser could not move the notification text, so a per-device pick would show `20:30` in the app and `8:30 PM` in the notification for the same event. |
